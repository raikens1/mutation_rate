...
# set wd
knitr::opts_knit$set(root.dir = 'C:/Users/VoightLab/Dropbox/SNP_rates/Analyses/chi_squared_tests/data')
...
removeX <- function(counts){
  remove <- as.character(nchar(counts$Context[1]))/2+2
  counts$chrX <- NULL
  counts$Count <- rowSums(count[,-c(1:remove)])
  return(counts)
}
```
...
```{r load_data, echo = F}
# load data
AFR_3mer_counts <- read.delim('AFR_3mer_counts.txt')
EUR_3mer_counts <- read.delim('EUR_3mer_counts.txt')
EAS_3mer_counts <- read.delim('EAS_3mer_counts.txt')
SAS_3mer_counts <- read.delim('SAS_3mer_counts.txt')

AFR_5mer_counts <- read.delim('AFR_5mer_counts.txt')
EUR_5mer_counts <- read.delim('EUR_5mer_counts.txt')
EAS_5mer_counts <- read.delim('EAS_5mer_counts.txt')
SAS_5mer_counts <- read.delim('SAS_5mer_counts.txt')

AFR_7mer_counts <- read.delim('AFR_7mer_counts.txt')
EUR_7mer_counts <- read.delim('EUR_7mer_counts.txt')
EAS_7mer_counts <- read.delim('EAS_7mer_counts.txt')
SAS_7mer_counts <- read.delim('SAS_7mer_counts.txt')
