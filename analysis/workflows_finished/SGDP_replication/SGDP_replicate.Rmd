...
ggplot(data = AFR_7mer_counts, aes(x = Count)) + geom_histogram() +
  scale_x_log10() +
  labs(title = "Africa", x = "Number of Observed Polymorphisms")

ggplot(data = SAS_7mer_counts, aes(x = Count)) + geom_histogram() +
  scale_x_log10() +
  labs(title = "South Asia", x = "Number of Observed Polymorphisms")
```
...
o.p.7mer <- ordered.p(p.7mer, filter = F)
...
table_3 <- left_join(rates, o.p.7mer) %>% 
  select(c(Context, AFR.relative.rate, EUR.relative.rate, SAS.relative.rate, EAS.relative.rate, p)) %>%
