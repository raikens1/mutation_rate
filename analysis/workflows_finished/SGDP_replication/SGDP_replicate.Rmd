...
```{r SGDP setup data}
AFR_3mer_counts <- read_delim("3mer/SGDP_AFR_subset_3mer_counts.txt", 
                              "\t", escape_double = FALSE, trim_ws = TRUE)
...
## 1.1 Test for Homogenity Across all Continental Groups
...
All p-values are nominally significant.  In addition, the relative infered mutation rates agree for many polymorphism types, with some exceptions:

```{r, eval = FALSE}
data.frame(Context = table1_SGDP$Context,
           "1KG relationship", 
           "SGDP relationship", 
           "Agreement")
```
