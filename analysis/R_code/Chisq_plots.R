...
volcano.plot <- function(counts.1, counts.2, p.vals, lab.lim){
  ...
  p.sigs <- subset(ps, p.vals$p < lab.lim)
  
  v_plot <- qplot(f.diff, -log10(p.vals$p)) +
    labs(x = "Fold excess in East Asia", y = expression(-log[10](italic(p))), title = "East Asia v. Africa")+
    scale_color_manual("", values = h.colors)+ #set colors
    geom_text(data = sigs, aes(f.diff.sigs, -log10(p.sigs$p), label = sigs$Context, color = sigs$One_mer), 
              vjust =0, nudge_y =.5, size = 3, check_overlap = TRUE)+ #label highly significant
    geom_point(aes(color=factor(counts.1$One_mer)), size = 3)+ #Color pts by one_mer
    geom_hline(yintercept = -log10(alpha), color = "black", linetype = 2)+#significance line
    geom_text(aes(-.45, -log10(alpha)+1, label = "significant (p < 5e-4)", vjust = 0), color = "black", size = 3)+#significant text
    theme(axis.text.x = element_text(size = rel(1.3)), axis.title.x = element_text(size = rel(1.3)), #adjust text sizes
          ...
          legend.text = element_text(size = rel(1.2)), legend.position = c(0.8, 0.75),
