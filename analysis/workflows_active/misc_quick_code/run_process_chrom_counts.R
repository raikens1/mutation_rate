...
cc <- read.delim("~/Documents/Research/mutation_rate/vcf_filtration/filtered_1kg_v3/PRIV_ANCESTRAL/bcftools_pipeline/1_filtered/EUR_all_chrom_counts.txt")

processed_cc <- process_chrom_counts(chrom_counts = cc,
                     gw_counts = gw_counts, 
                     subcontext_ref = ref)

write.table(processed_cc, "3mer/EUR_all_3mer_counts.txt", quote = F, sep = "\t", row.names = F)