...
cc <- read.delim("~/Documents/Research/mutation_rate/vcf_filtration/SGDP/raw_file_prep/3_private/SGDP_AFR_subset_chrom_counts.txt")

processed_cc <- process_chrom_counts(chrom_counts = cc,
                     gw_counts = gw_counts, 
                     subcontext_ref = ref)

write.table(processed_cc, "3mer/SGDP_AFR_subset_3mer_counts.txt", quote = F, sep = "\t", row.names = F)